-- migration_v7_remaining.sql — Steps 3-7 (1-2 already applied)
USE mydb;
SET FOREIGN_KEY_CHECKS = 0;

-- ─── 3. bull_match_history: PK → (match_id, bull_id) ────────
-- De-duplicate: keep only the lowest round
...
-- Drop old PK — must drop FKs first
ALTER TABLE bull_match_history DROP FOREIGN KEY fk_bull_match_history_bull_table1;
ALTER TABLE bull_match_history DROP FOREIGN KEY fk_bull_match_history_Match1;
ALTER TABLE bull_match_history DROP FOREIGN KEY fk_bull_match_history_player1;
ALTER TABLE bull_match_history DROP FOREIGN KEY fk_bull_match_history_prize1;
ALTER TABLE bull_match_history DROP FOREIGN KEY fk_bull_match_history_round_type1;
...
-- Re-add FKs
ALTER TABLE bull_match_history
  ADD CONSTRAINT fk_bmh_match FOREIGN KEY (match_id) REFERENCES `match`(match_id),
  ADD CONSTRAINT fk_bmh_bull FOREIGN KEY (bull_id) REFERENCES bull_table(bull_id),
  ADD CONSTRAINT fk_bmh_player FOREIGN KEY (player_id) REFERENCES player(player_id),
  ADD CONSTRAINT fk_bmh_prize FOREIGN KEY (prize_id) REFERENCES prize(prize_id),
  ADD CONSTRAINT fk_bmh_round FOREIGN KEY (round_type_id) REFERENCES round_type(round_type_id);

-- Batch FK
ALTER TABLE bull_match_history ADD CONSTRAINT fk_bmh_batch
  FOREIGN KEY (batch_id) REFERENCES batch(batch_id);

-- ─── 4. match_round_config ───────────────────────────────────
